...
const themes = ["Black", "Pink", "Purple", "White"];
...
document.getElementById('exit-button').addEventListener('click', function () {
  ...
  fetch('http://localhost:8000/saveCSVData/', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
document.getElementById('license-button').addEventListener('click', function () {
  ...
  // POST CSV data to the server
  fetch(`http://localhost:8000/validateLicenseKey/${license_key}`, {
    headers: {
      'Content-Type': 'application/json',
    }
  })
...
fetch('http://localhost:8000/licenseKeyIsValid/', {
  method: 'GET',
  headers: {
  'Content-Type': 'application/json',
  }

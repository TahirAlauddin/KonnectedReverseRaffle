...
document.addEventListener("DOMContentLoaded", function () {
    ...
    for (var i = 0; i < Math.ceil(totalNumbers / columns); i++) {
      for (var j = 0; j < columns && count < totalNumbers; j++) {
        ...
        if (clickable) {
          ...
          if (window.getComputedStyle(document.getElementById("number-panel-grid")).getPropertyValue('background-image') !== 'url("http://localhost:8000/home/null")') {
  ...
  fetch('http://localhost:8000/getAdminConf/')
  .then(response => response.json())
  .then(data => {
